...
path = r'G:\mogodar_case\case45\void_show_copy.xlsx'
data = pd.read_excel(path, header=None)
# data.iloc[:,:] = data.iloc[:,:]*1.2
# 将数据转换为二维数组
...
# # 添加等温线
# contours = plt.contour(temperature_matrix, colors='black', linewidths=0.5, linestyles='solid',levels=[0.2,0.3,0.4,0.5])
# plt.clabel(contours, inline=True, fontsize=14, fmt='%.2f',inline_spacing=5)
...
# 添加标题和轴标签
ax = plt.gca()
plt.text(1.03, 0.97, 'Porosity (-)',transform=ax.transAxes,fontsize=12)
x = []
